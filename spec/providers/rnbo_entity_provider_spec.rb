...
RSpec.describe RnboProvider, tdd: true do
  ...
  let!(:rnbo) { RnboEntityProvider.new }


  describe 'validate entities data' do
    let(:validate_data) { rnbo.validate_entities }
    ...
    it 'keys should be equal to factory sample' do
      expect(validate_data_instance.keys).to eq(rnbo_entity_person.keys)
    end
